...
const AddCompanyModal: React.FC<AddCompanyModalProps> = ({ open, onClose, onSuccess, initialData, companyId }) => {
  ...
  useEffect(() => {
    const fetchFiscalYears = async () => {
      ...
      try {
        const res = await axios.get<FiscalYear[]>('http://43.205.239.123/api/settings/fiscal-year/',
          {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = companyId
        ? `http://43.205.239.123/api/inventory/supplier-registry/${companyId}/`
        : 'http://43.205.239.123/api/inventory/supplier-registry/';
