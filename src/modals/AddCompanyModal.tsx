...
const AddCompanyModal: React.FC<AddCompanyModalProps> = ({ open, onClose, onSuccess, initialData, companyId }) => {
  ...
  useEffect(() => {
    const fetchFiscalYears = async () => {
      ...
      try {
        const res = await axios.get<FiscalYear[]>('http://213.199.53.33:81/api/settings/fiscal-year/',
          {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = companyId
        ? `http://213.199.53.33:81/api/inventory/supplier-registry/${companyId}/`
        : 'http://213.199.53.33:81/api/inventory/supplier-registry/';
