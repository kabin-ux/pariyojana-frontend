    ...
    const fetchDashboardData = async () => {
      ...
      try {
        const [summaryRes, budgetRes, areaRes, wardRes, budgetAreaRes] = await Promise.all([
          ...
          axios.get('http://localhost:8000/api/dashboard/budget-summary/', {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get('http://localhost:8000/api/dashboard/area-wise-distribution/', {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get('http://localhost:8000/api/dashboard/wardwise-budget/', {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get('http://localhost:8000/api/dashboard/budget-area/', {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
