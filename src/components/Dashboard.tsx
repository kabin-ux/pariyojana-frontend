    ...
    const fetchDashboardData = async () => {
      ...
      try {
        const [summaryRes, budgetRes, areaRes, wardRes, budgetAreaRes] = await Promise.all([
          axios.get('http://43.205.255.142/api/dashboard/summary/', {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get('http://43.205.255.142/api/dashboard/budget-summary/', {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get('http://43.205.255.142/api/dashboard/area-wise-distribution/', {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get('http://43.205.255.142/api/dashboard/wardwise-budget/', {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get('http://43.205.255.142/api/dashboard/budget-area/', {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
