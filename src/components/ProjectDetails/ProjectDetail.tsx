...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ project, onBack }) => {
  ...
  const handleSave = async (data: any) => {
    try {
      ...
      if (committeeDetail) {
        const url = `http://43.205.255.142/api/projects/${project.serial_number}/consumer-committee-details/${committeeDetail.id}/`;
        await axios.patch(url, data, config);
        toast.success("समिति विवरण सफलतापूर्वक अपडेट गरियो");
      } else {
        const url = `http://43.205.255.142/api/projects/${project.serial_number}/consumer-committee-details/`;
  ...
  const handleSavePosition = async (rows: FormRow[]) => {
    try {
      ...
      const updatePromises = rowsToUpdate.map(async (row) => {
        ...
        const url = `http://43.205.255.142/api/projects/${project.serial_number}/official-details/${matchedDetail?.id}/`;
  ...
  const handleSaveResearch = async (rows: FormRow[]) => {
    try {
      ...
      const updatePromises = rowsToUpdate.map(async (row) => {
        ...
        const url = `http://43.205.255.142/api/projects/${project.serial_number}/monitoring-committee/${matchedDetail?.id}/`;
  ...
  const handleFileUpload = async (data: any) => {
    try {
      ...
      let url = `http://43.205.255.142/api/projects/${projectIdNum}/map-cost-estimate/`;
      let method = 'post';

      if (data.editMapCostId) {
        url = `http://43.205.255.142/api/projects/${projectIdNum}/map-cost-estimate/${data.editMapCostId}/`;
        method = 'patch';
      }
  ...
  const handleDownload = async (itemSerialNo: number, projectSerialNo: number) => {
    try {
      const response = await axios.get(
        `http://43.205.255.142/api/projects/consumer-committee/generate-pdf/${itemSerialNo}/${projectSerialNo}/`,
        {
          responseType: 'blob',
        }
      );
  ...
  const handleDownloadProjectAgreement = async (itemSerialNo: number, projectSerialNo: number) => {
    try {
      const response = await axios.get(
        `http://43.205.255.142/api/projects/project-plan-tracker/download/${itemSerialNo}/${projectSerialNo}/`,
        {
          responseType: 'blob',
        }
      );
  ...
  const handleDownloadProjectAgreementAndWorkLoad = async (itemSerialNo: number, projectSerialNo: number) => {
    try {
      const response = await axios.get(
        `http://43.205.255.142/api/projects/project-aggrement/download/${itemSerialNo}/${projectSerialNo}/`,
        {
          responseType: 'blob',
        }
      );
  ...
  const handleAddProjectAgreement = async (data: any) => {
    try {
      ...
      if (agreementDetail) {
        const url = `http://43.205.255.142/api/projects/${project.serial_number}/project-agreement-details/${agreementDetail.id}/`;
        await axios.patch(url, data, config);
        toast.success("समिति विवरण सफलतापूर्वक अपडेट गरियो");
      } else {
        const url = `http://43.205.255.142/api/projects/${project.serial_number}/project-agreement-details/`;
        await axios.post(url, { ...data, project: project.serial_number }, config);
        toast.success("समिति विवरण सफलतापूर्वक थपियो");
      }
  ...
  const handleDownloadOtherDocument = async (itemSerialNo: number, projectSerialNo: number) => {
    try {
      const response = await axios.get(
        `http://43.205.255.142/api/projects/other-documents/download/${itemSerialNo}/${projectSerialNo}/`,
        {
          responseType: 'blob',
        }
      );
  ...
  const handleAddDocument = async (formPayload: FormData) => {
    try {
      ...
      const url = `http://43.205.255.142/api/projects/${project.serial_number}/documents/`;
  ...
  const handleEditDocument = async (data: any) => {
    try {
      ...
      const url = `http://43.205.255.142/api/projects/${project.serial_number}/documents/${documentDetail.id}/`;
  ...
  if (result.isConfirmed) {
    try {
      const token = localStorage.getItem('access_token');
      await axios.delete(`http://43.205.255.142/api/projects/${project.serial_number}/documents/${id}/`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleGenerateBill = async (projectId: number) => {
    try {
      const response = await fetch(`http://43.205.255.142/api/projects/bill/project/${projectId}/pdf/`, {
        method: 'GET',
        headers: {
