...
const Inventory: React.FC = () => {
  ...
  const fetchInventory = async () => {
    try {
      ...
      const response = await axios.get('http://3.108.67.147/api/inventory/supplier-registry/', {
  ...
  const handleEditClick = async (id: number) => {
    try {
      const token = localStorage.getItem('access_token');
      const response = await axios.get(`http://3.108.67.147/api/inventory/supplier-registry/${id}/`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      await axios.delete(`http://3.108.67.147/api/inventory/supplier-registry/${id}/`, {
        headers: { Authorization: `Bearer ${token}` }
      });
