...
const SubjectCommittee: React.FC = () => {
  ...
  const handlePrioritizeThematicWardProject = async (id: number) => {
    try {
      await axios.post(`https://www.bardagoriyapms.com/api/planning/thematic/wardrecommend-project/${id}/prioritize/`);
  ...
  const handlePrioritize = async (id: number) => {
    try {
      await axios.post(`https://www.bardagoriyapms.com/api/planning/thematic/thematic-plans/${id}/prioritize/`);
      toast.success('परियोजना सफलतापूर्वक प्राथमिकरण गरियो।');
      window.location.reload();
    } catch (error) {
      console.error(error);
  ...
  const handleRecommendtoBudget = async (id: number) => {
    try {
      await axios.post(`https://www.bardagoriyapms.com/api/planning/thematic/prioritized-plans/${id}/submit-to-budget-committee/`);
