...
export const ProjectsTable = ({
  ...
  const handlePrioritize = async (id: number) => {
    ...
    if (tabType === 'वडा स्तरीय परियोजना') {
      endpoint = `http://213.199.53.33:8001/api/planning/ward-office/ward-projects/${id}/prioritize/`;
    } else if (tabType === 'विषयगत समितिका परियोजना') {
      endpoint = `http://213.199.53.33:8001/api/planning/ward-office/ward-thematic-projects/${id}/prioritize/`;
    } else {
      return;
    }
  ...
  const handleRecommendtoBudget = async (id: number) => {
    try {
      await axios.post(`http://213.199.53.33:8001/api/planning/ward-office/prioritized-ward-projects/${id}/recommend-to-budget-committee/`);
  ...
  const handleRecommendMunicipalProgramToBudget = async (id: number) => {
    try {
      await axios.post(`http://213.199.53.33:8001/api/planning/ward-office/municipality-projects/${id}/recommend-to-budget-committee/`);
      toast.success('परियोजना सफलतापूर्वक बजेट तथा कार्यक्रम तर्जुमा समितिमा सिफारिस गरियो।');
      refetch?.();
    } catch (error) {
      console.error('Error recommending to budget committee:', error);
  ...
  const handleRecommendtoThematic = async (id: number) => {
    try {
      await axios.post(`http://213.199.53.33:8001/api/planning/ward-office/prioritized-ward-thematic/${id}/recommend-to-ward-projects/`);
