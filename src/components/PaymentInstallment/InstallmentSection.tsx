...
const InstallmentSection: React.FC<InstallmentSectionProps> = ({
    ...
    const handleFileUpload = async (file: File, serial_no: number) => {
        try {
            ...
            await axios.post(
                `http://213.199.53.33:81/api/projects/${endpoint}/${project.serial_number}/upload/`,
                formData,
                {
                    headers: {
    ...
    const handleDownloadPDF = async (serial_no: number) => {
        try {
            ...
            const response = await axios.get(
                `http://213.199.53.33:81/api/projects/${endpoint}/generate-pdf/${serial_no}/${project.serial_number}/`,
                { responseType: 'blob' }
            );
