  ...
  const today = new Date();
    ...
    const savePaymentDetails = async (data: any) => {
        try {
            ...
            const url = `http://3.108.67.147/api/projects/${project.serial_number}/payment-details/`;
    ...
    const handlePayment = async (projectId: number) => {
        try {
            ...
            const response = await axios.get(
                `http://3.108.67.147/api/projects/installment/payment/project/${projectId}/pdf/`,
                {
                    responseType: 'blob',
                    headers: {
                        'Authorization': `Bearer ${token}`
