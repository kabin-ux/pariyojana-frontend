  ...
  const today = new Date();
    ...
    const savePaymentDetails = async (data: any) => {
        try {
            ...
            const url = `http://213.199.53.33:8001/api/projects/${project.serial_number}/payment-details/`;
    ...
    const handlePayment = async (projectId: number) => {
        try {
            ...
            const response = await axios.get(
                `http://213.199.53.33:8001/api/projects/installment/payment/project/${projectId}/pdf/`,
                {
                    responseType: 'blob',
                    headers: {
                        'Authorization': `Bearer ${token}`
