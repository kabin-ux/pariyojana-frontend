...
const BankDetailsSection: React.FC<BankDetailsSectionProps> = ({
    ...
    const fetchBankDetails = async (projectId: number) => {
        try {
            ...
            const response = await axios.get(
                `/api/projects/${projectId}/bank-details/`,
                {
                    headers: {
    ...
    const fetchBankRecommendationDetails = async (projectId: number) => {
        try {
            const token = localStorage.getItem('access_token');
            const response = await axios.get(
                `/api/projects/${projectId}/bank-account-recommendation/`,
                {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
    ...
    const fetchBankAccountPhotos = async (projectId: number) => {
        try {
            const token = localStorage.getItem('access_token');
            const response = await axios.get(
                `/api/projects/${projectId}/account-photos/`,
                {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
    ...
    const saveBankRecommendation = async (data: any) => {
        try {
            ...
            let url = `/api/projects/${project.serial_number}/bank-account-recommendation/`;
            let method = 'post';

            if (editRecommendationId) {
                url = `/api/projects/${project.serial_number}/bank-account-recommendation/${editRecommendationId}/`;
                method = 'patch';
            }
    ...
    const saveAccountPhoto = async (data: any) => {
        try {
            ...
            let url = `/api/projects/${project.serial_number}/account-photos/`;
            let method: 'post' | 'patch' = 'post';

            if (editAccountPhotoId) {
                url = `/api/projects/${project.serial_number}/account-photos/${editAccountPhotoId}/`;
                method = 'patch';
            }
    ...
    const addBankDetails = async (projectSerialNumber: number, data: {
        ...
        try {
            const token = localStorage.getItem('access_token');
            let url = `/api/projects/${projectSerialNumber}/bank-details/`;
            let method = 'post';
            if (editBankId) {
                url = `/api/projects/${projectSerialNumber}/bank-details/${editBankId}/`;
                method = 'patch';
            }
            await axios({
                url,
